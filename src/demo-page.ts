...
import { animateScrollToBottom, cancelScrollAnimation, isAtBottom } from './utils/animate-scroll';
...
export class DemoPage extends LitElement {
  ...
  override firstUpdated(): void {
    ...
    this._viewerContainer.addEventListener('wheel', this._handleWheel, { passive: true });
    this._viewerContainer.addEventListener('touchstart', this._handleTouchStart, { passive: true });
    this._viewerContainer.addEventListener('touchmove', this._handleTouchMove, { passive: true });
    this._viewerContainer.addEventListener('scroll', this._handleScroll, { passive: true });
  ...
  override disconnectedCallback(): void {
    ...
    if (this._viewerContainer) {
      ...
      this._viewerContainer.removeEventListener('touchstart', this._handleTouchStart);
      this._viewerContainer.removeEventListener('touchmove', this._handleTouchMove);
  ...
  private _updateScrollButtonVisibility(): void {
    ...
    const isScrollable = this._viewerContainer.scrollHeight > this._viewerContainer.clientHeight;
  ...
  override render() {
    const streamingStatus = this._isPaused
      ...
      : this._isStreaming
        ? 'Streaming...'
        : 'Idle';
    const progress = sampleMarkdown.length > 0
    ...
    return html`
      <div class="container">
        <header>
          ...
          <p class="subtitle">Lit Web Component port of the typefm Svelte 5 markdown viewer</p>
        </header>
        ...
        <div class="viewer-wrapper">
          ...
          <button class="scroll-to-bottom" @click=${this._scrollToBottomAndEnable} title="Scroll to bottom">
